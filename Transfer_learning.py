...
imshow(show,title = [class_names[x] for x in classes])   
...
def train_model(model,criterion,optimizer,scheduler,num_epochs=25):
    since = time.time()
    best_wtc=copy.deepcopy(model.state_dict())
    best_acc=0.0
    for epoch in range(num_epochs):
    ...
    model.load_state_dict(best_wtc)
    return model
...
model=train_model(model,criterion,optimizer,exp_lr_scheduler,num_eopchs=25)
